...
impl Steps {
  pub fn create(steps_v: Vec<Step>, input_v: Box<Input>) -> Box<Input> {
    Box::new(Steps { input : input_v, steps : steps_v })
  }
}
...
impl Input for Steps {
  fn compute(&mut self) -> f64 {
    ...
    if input <= min_step.v_in { return min_step.v_out; }
    if input >= max_step.v_in { return max_step.v_out; }
